...
# run kafka
/opt/kafka_2.13-2.5.1/bin/kafka-server-start.sh /opt/kafka_2.13-2.5.1/config/server.properties --override broker.id=${BROKER_ID} --override listeners=PLAINTEXT://0.0.0.0:${KAFKA_SERVICE_PORT} --override advertised.listeners=PLAINTEXT://${KAFKA_SERVICE_PUBLIC_HOST}:${KAFKA_SERVICE_PUBLIC_PORT} --override zookeeper.connect=${ZOOKEEPER_SERVICE_HOST}:${ZOOKEEPER_SERVICE_PORT} --override controlled.shutdown.enable=true
#tail -f /dev/null
