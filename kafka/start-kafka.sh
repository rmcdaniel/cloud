...
# run kafka
/opt/kafka_2.13-2.5.1/bin/kafka-server-start.sh /opt/kafka_2.13-2.5.1/config/server.properties --override broker.id=${BROKER_ID} --override listeners=INTERNAL://${KAFKA_SERVICE_PRIVATE_HOST}:${KAFKA_SERVICE_PORT_INTERNAL},EXTERNAL://0.0.0.0:${KAFKA_SERVICE_PORT_EXTERNAL} --override advertised.listeners=INTERNAL://${KAFKA_SERVICE_PRIVATE_HOST}:${KAFKA_SERVICE_PORT_INTERNAL},EXTERNAL://${KAFKA_SERVICE_PUBLIC_HOST}:${KAFKA_SERVICE_PUBLIC_PORT} --override zookeeper.connect=${ZOOKEEPER_SERVICE_HOST}:${ZOOKEEPER_SERVICE_PORT} --override inter.broker.listener.name=INTERNAL --override listener.security.protocol.map=INTERNAL:PLAINTEXT,EXTERNAL:PLAINTEXT
#tail -f /dev/null
